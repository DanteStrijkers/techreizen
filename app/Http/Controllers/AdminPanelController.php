...
class AdminPanelController extends Controller
{
    public function index()
    {
        ...
        $trips = Trip::withCount('travellers as participants_count')->get();
    ...
    public function sendTripMessage(Request $request): RedirectResponse
    {
        ...
        $trip = Trip::with('travellers')->find($validated['trip_id']);
        $admin = auth()->user();
        ...
        try {
            foreach ($trip->travellers as $participant) {
            ...
            return back()->with(
                'success',
                "Bericht verzonden naar {$trip->travellers->count()} deelnemers van {$trip->name}"
            );
        } catch (\Exception $e) {
